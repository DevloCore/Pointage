...
onMounted(() => {
	observer = new ResizeObserver(([entry]) => {
		containerHeight.value = entry.contentRect.height + 16 // 16 de padding
	})
...
<template>
	<!-- Outer : hauteur animée -->
	<div class="overflow-hidden transition-[height] duration-300 ease-in-out"
		:style="{ height: containerHeight + 'px' }">
		<!-- Inner : mesuré par le ResizeObserver -->
		<div ref="inner" class="max-w-lg mx-auto flex flex-col items-center justify-center relative">
			...
			<transition name="fade" mode="out-in">
				<div v-if="step === 'initial'" key="initial" class="flex flex-col items-center">
					...
					<div class="pointage-btn-wrapper">
						...
						<button @click="onFirstClick" @pointerdown="pressing = true" @pointerup="pressing = false"
							@pointerleave="pressing = false" class="pointage-btn"
					...
					<div class="flex items-center gap-4 mb-8">
						...
						<div class="flex flex-col items-center">
							...
							<select v-model.number="selectedMinute"
								class="bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-xl px-4 py-3 text-2xl font-mono text-center appearance-none focus:outline-none focus:ring-2 focus:ring-indigo-500">
								<option v-for="m in minutes" :key="m" :value="m">{{ String(m).padStart(2, '0') }}
								</option>
							</select>
						</div>
					</div>
