...
import { getPointagesByDateRange, getSetting } from '../db'
import { useSync } from '../composables/useSync'
import PointageView from '../components/PointageView.vue'
import { CloudIcon, Cog6ToothIcon, ChartBarIcon, ClockIcon } from '@heroicons/vue/24/outline'
...
const handlePointageUpdate = async () => {
	if (today) {
		todayPointages.value = await getPointagesByDateRange(today, today)
	}
	if (weekDates.length > 0) {
		weekPointages.value = await getPointagesByDateRange(weekDates[0], weekDates[weekDates.length - 1])
	}
}
...
<template>
	<div class="max-w-lg mx-auto px-4 pt-6">
		...
		<Presentation v-if="!isConfigured" />
		...
		<template v-else>
			...
			<div class="bg-white dark:bg-gray-800 rounded-2xl shadow p-5 mb-4">
				...
				<div class="mt-3">
					...
					<div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
						<div class="bg-cyan-600 dark:bg-cyan-400 h-2.5 rounded-full transition-all duration-500"
			...
			<!-- Week -->
			<div class="bg-white dark:bg-gray-800 rounded-2xl shadow p-5 mb-4">
				<h2 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Cette
					semaine</h2>
			...
			<div class="bg-white dark:bg-gray-800 rounded-2xl shadow p-5">
				<h2 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Delta
					semaine</h2>
				<p class="text-3xl font-bold" :class="weekDelta >= 0 ? 'text-green-500' : 'text-red-500'">
